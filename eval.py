...
from src.tools.callback import EvaluateCallBack,VarMonitor,VccCallback
...
def main():
    ...
    model = Model(net_with_loss, 
                metrics={'top_1_accuracy', 'top_5_accuracy', "loss"},  # "acc",'top_1_accuracy', 'top_5_accuracy'
                eval_network=eval_network,
                eval_indexes=eval_indexes)

    start = time.time()
    
    res = model.eval(data.val_dataset, callbacks=[VccCallback(batch_num = data.val_dataset.get_dataset_size())])
    print("result: ", res)
    end = time.time()
